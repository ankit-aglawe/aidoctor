...
```
NO PYTHON EMITTED WITHOUT THE 5-POINT PRE-FLIGHT PASSING
```

If any of the five checks below is unclear, resolve it **before** generating code. "I'll fix it after" is the failure mode this rule exists to prevent. **Spirit over letter** — paraphrasing the pre-flight ("I'll just glance at it") is the same violation as skipping it.
...
| "I'll add a TODO to come back to this" | Bare `# TODO` rots. Attach a ticket (`GH-123`, `JIRA-456`, URL) or implement now. |
| "This is just a test / example / script, the rules don't apply" | The rules apply. CI runs against tests too. |
| "I'll wrap it in `try: except Exception:` to be safe" | That's the opposite of safe. It hides the bug you'd have fixed. |
| "I don't know the exact type so I'll use `Any`" | Read the code, infer the type, or use a `Protocol`/`TypeVar`. `Any` is a last resort. |
| "It's only a small block, the violation doesn't matter" | `aidoctor scan` doesn't grade on size. Zero means zero. |
